...
export default function HeroSection() {
  ...
  // Image carousel data - alternating between before/after and original
  const images = [
    {
  ...
  // Dynamic card content based on current image
  const getCardContent = () => {
    if (currentImageIndex === 0) {
      ...
      return {
        ...
        right: { icon: Clock, value: "24/7", label: "Access", bgColor: "bg-primary/20", iconColor: "text-primary" },
      }
    } else if (currentImageIndex === 1 || currentImageIndex === 2) {
      // Fat to lean transformation
      return {
        left: {
          icon: Zap,
      ...
      // Lean to muscular transformation
      return {
        left: {
          icon: Zap,
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl w-full">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 xl:grid-cols-3 gap-8 md:gap-8 lg:gap-8 items-center justify-items-center">
          ...
          <div
            className={`w-full md:col-span-1 lg:col-span-3 xl:col-span-2 space-y-6 z-10 relative text-center md:text-left lg:text-left transition-all duration-1000 ease-out ${
              isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}
          >
            <div className="space-y-4">
              <div
                className={`flex justify-center md:justify-start lg:justify-start transition-all duration-1000 ease-out delay-200 ${
              ...
              <h1
                className={`text-5xl md:text-6xl font-bold text-balance leading-tight tracking-tight text-foreground transition-all duration-1000 ease-out delay-400 ${
                  isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                }`}
              >
                Build Your
              ...
              <p
                className={`text-base sm:text-lg md:text-lg lg:text-xl text-muted-foreground text-pretty max-w-xl leading-relaxed mx-auto md:mx-0 lg:mx-0 transition-all duration-1000 ease-out delay-600 ${
            ...
            <div
              className={`flex justify-center md:justify-start lg:justify-start pt-2 transition-all duration-1000 ease-out delay-800 ${
                isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
              }`}
            >
              <Button
                ...
                className="bg-accent cursor-pointer font-bold hover:bg-accent/90 active:bg-accent/80 
                            ...
                            shadow-md hover:shado  transition-all duration-200 
                            max-w-sm sm:w-auto"
              >
          ...
          <div
            className={`w-full md:col-span-1 lg:col-span-2 xl:col-span-1 relative flex justify-center transition-all duration-1200 ease-out delay-300 ${
              isLoaded ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8"
            }`}
          >
            {/* Background blur elements - adjusted for mobile */}
            <div
              className={`absolute top-4 sm:top-8 md:top-12 lg:top-16 right-4 sm:right-8 md:right-10 lg:right-12 w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 lg:w-32 lg:h-32 bg-accent/10 rounded-full blur-2xl transition-all duration-1500 ease-out delay-1000 ${
                isLoaded ? "opacity-100 scale-100" : "opacity-0 scale-75"
              }`}
            ></div>
            <div
              className={`absolute bottom-8 sm:bottom-16 md:bottom-18 lg:bottom-24 left-4 sm:left-6 md:left-8 lg:left-10 w-10 h-10 sm:w-14 sm:h-14 md:w-16 md:h-16 lg:w-24 lg:h-24 bg-primary/20 rounded-full blur-xl transition-all duration-1500 ease-out delay-1200 ${
                isLoaded ? "opacity-100 scale-100" : "opacity-0 scale-75"
              }`}
            ></div>
            ...
            <div className="relative w-full max-w-[280px] sm:max-w-xs md:max-w-sm lg:max-w-lg mx-auto">
              ...
              {/* Main carousel image with proper mobile height */}
              <div className="relative   h-[45vh] sm:h-[50vh] mx-4  md:h-[55vh] lg:h-[70vh]">
                <img
                  src={currentImage.src || "/placeholder.svg"}
                  alt={currentImage.alt}
                  className="relative z-10  w-full h-full object-contain object-bottom drop-shadow-2xl transition-opacity duration-500 rounded-2xl"
                />
              ...
              {/* Stats cards - repositioned for mobile */}
              <Card
                className={`absolute top-2 -left-2 sm:top-4 sm:-left-4 md:top-4 md:-left-3 lg:top-8 lg:-left-8 bg-card border-2 border-border shadow-xl transform transition-all duration-1000 hover:scale-105 hover:shadow-2xl ease-out delay-700 ${
              ...
              >
                <CardContent className="p-2 sm:p-3 md:p-2.5 lg:p-4">
                  <div className="flex items-center gap-2 sm:gap-3 md:gap-2 lg:gap-3">
                    <div
                      className={`w-6 h-6 sm:w-8 sm:h-8 md:w-7 md:h-7 lg:w-10 lg:h-10 ${cardContent.left.bgColor} rounded-lg sm:rounded-xl flex items-center justify-center transition-all duration-500 animate-pulse`}
                    >
              ...
              <Card
                className={`absolute bottom-4 -right-2 sm:bottom-6 sm:-right-4 md:bottom-6 md:-right-3 lg:bottom-12 lg:-right-10 bg-card/95 backdrop-blur-sm border border-border/50 shadow-lg transform transition-all duration-1000 ease-out hover:scale-110 hover:shadow-xl hover:-translate-y-1 delay-900 ${
                  isLoaded ? "opacity-100 translate-x-0 translate-y-0" : "opacity-0 translate-x-8 translate-y-4"
                }`}
              >
                <CardContent className="p-2 sm:p-3 md:p-2.5 lg:p-4">
